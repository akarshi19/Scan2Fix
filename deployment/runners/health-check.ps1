...
try {
    ...
    if ($scan2fix -and $scan2fix.pm2_env.status -eq "online") {
        Write-Host "    OK: Online (PID: $($scan2fix.pid))" -ForegroundColor Green
    ...
    Write-Host "    ERROR: PM2 not responding" -ForegroundColor Red
    $allGood = $false
}
Write-Host ""
...
if ($ngrokProcess) {
    Write-Host "    OK: Running (PID: $($ngrokProcess.Id))" -ForegroundColor Green
    ...
    try {
        ...
        Write-Host "    WARNING: Could not get tunnel URL" -ForegroundColor Yellow
    }
} else {
    Write-Host "    ERROR: NOT Running" -ForegroundColor Red
    $allGood = $false
...
try {
    ...
    Write-Host "    WARNING: Could not get database stats" -ForegroundColor Yellow
}
Write-Host ""
...
if ($allGood) {
    ...
    Write-Host "  WARNING: SOME SERVICES NEED ATTENTION" -ForegroundColor Red
