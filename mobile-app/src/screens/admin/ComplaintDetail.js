...
export default function ComplaintDetail({ route, navigation }) {
  ...
  const [selectedStaff, setSelectedStaff] = useState(complaint.assigned_staff_id || '');
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Staff Assigned! ✅',
        `This complaint has been assigned to ${staffMember?.full_name || staffMember?.email}`,
        ...
        {complaint.status === 'CLOSED' ? (
          <View style={styles.closedBanner}>
            ...
            <Text style={styles.closedDate}>
              Closed on: {new Date(complaint.closed_at).toLocaleDateString()}
            </Text>
          </View>
        ) : (
          <>
            ...
            {staffList.length === 0 && (
              <Text style={styles.noStaffText}>
                No staff members found. Please add staff users first.
              </Text>
            )}
            ...
            {complaint.assigned_staff_id && (
              <Text style={styles.currentAssignment}>
                Currently assigned to: {
                  staffList.find(s => s.id === complaint.assigned_staff_id)?.full_name || 
                  'Unknown'
                }
              </Text>
            )}
