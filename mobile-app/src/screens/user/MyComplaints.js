...
//   Server handles all joins and returns same shape
// ============================================
    ...
    <View style={styles.card}>
      ...
      {item.photo_url && (
        <TouchableOpacity onPress={() => setSelectedImage(getFileUrl(item.photo_url))}>
          <Image source={{ uri: getFileUrl(item.photo_url) }} style={styles.thumbnail} />
      ...
      {item.closed_at && (
        <Text style={styles.closedDate}>✅ Resolved: {new Date(item.closed_at).toLocaleDateString()}</Text>
      )}
    </View>
  );
    ...
    <View style={styles.container}>
      <View style={styles.tabContainer}>
        <FlatList
          ...
          data={tabs} keyExtractor={(item) => item.key}
          renderItem={({ item }) => (
            <TouchableOpacity
              style={[styles.tab, activeTab === item.key && { borderBottomColor: item.color, borderBottomWidth: 3 }]}
              onPress={() => setActiveTab(item.key)}
            >
              <Text style={styles.tabIcon}>{item.icon}</Text>
              <Text style={[styles.tabLabel, activeTab === item.key && { color: item.color, fontWeight: 'bold' }]}>
                {item.label}
              </Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            <Text style={styles.emptyIcon}>{activeTab === 'ALL' ? '📭' : activeTab === 'OPEN' ? '🟡' : activeTab === 'ASSIGNED' ? '🔵' : activeTab === 'IN_PROGRESS' ? '🟣' : '✅'}</Text>
            ...
            <Text style={styles.emptyText}>
              {activeTab === 'ALL' ? 'Your submitted complaints will appear here' : `Complaints with "${activeTab}" status will appear here`}
            </Text>
          </View>
        }
      />
...
const styles = StyleSheet.create({
  ...
  tabContainer: { backgroundColor: 'white', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 2 },
  tab: { alignItems: 'center', paddingVertical: 12, paddingHorizontal: 16, borderBottomWidth: 3, borderBottomColor: 'transparent', minWidth: 80 },
  tabIcon: { fontSize: 18, marginBottom: 4 },
  tabLabel: { fontSize: 11, color: '#666' },
  tabBadge: { position: 'absolute', top: 5, right: 5, paddingHorizontal: 6, paddingVertical: 2, borderRadius: 10, minWidth: 18, alignItems: 'center' },
  ...
  location: { color: '#666', marginBottom: 8, fontSize: 13 },
  description: { color: '#333', marginBottom: 10, lineHeight: 20 },
  staffCard: { backgroundColor: '#e3f2fd', padding: 10, borderRadius: 8, marginBottom: 10 },
  staffRow: { flexDirection: 'row', alignItems: 'center' },
  staffPhoto: { width: 40, height: 40, borderRadius: 20, marginRight: 10 },
  staffPhotoPlaceholder: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#2196F3', justifyContent: 'center', alignItems: 'center', marginRight: 10 },
  staffInitial: { color: 'white', fontWeight: 'bold', fontSize: 16 },
  staffInfo: { flex: 1 },
  staffLabel: { fontSize: 11, color: '#666' },
  staffName: { fontSize: 14, fontWeight: 'bold', color: '#333', marginTop: 2 },
  thumbnail: { width: '100%', height: 150, borderRadius: 8, marginBottom: 5 },
  tapToView: { fontSize: 11, color: '#2196F3', textAlign: 'center', marginBottom: 10 },
  verifyButton: { backgroundColor: '#4CAF50', padding: 15, borderRadius: 8, alignItems: 'center', marginBottom: 10 },
  verifyButtonText: { color: 'white', fontWeight: 'bold', fontSize: 14 },
  date: { color: '#999', fontSize: 12 },
  closedDate: { color: '#4CAF50', fontSize: 12, marginTop: 5 },
