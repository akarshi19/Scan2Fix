...
export default function UserDashboard({ navigation }) {
  ...
  const s = StyleSheet.create({
    scanButton: {
      ...
      shadowOpacity: 0.3, shadowRadius: 12, elevation: 8, borderColor: colors.textPri,
    },
    scanTitle: { fontSize: 18, fontWeight: '700', color: colors.textPri, marginTop: 10 },
    scanSubtitle: { fontSize: 8, color: colors.textSec, marginTop: 5 },
    infoCard: {
  ...
  return (
    <ScreenLayout title={`${t('welcome')}, ${user?.full_name || t('user')}!`} showDecor>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.85}
      >
        <Ionicons name="qr-code-outline" size={50} color={colors.textPri} />
