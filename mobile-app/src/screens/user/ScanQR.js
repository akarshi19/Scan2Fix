...
/**
 * ScanQR - QR Code Scanner using device camera
 * 
 * FLOW:
 * 1. Request camera permission
 * 2. Show camera with QR scanner overlay
 * 3. When QR is scanned, extract asset ID
 * 4. Look up asset in database
 * 5. Navigate to complaint form with asset details
 * 
 * QR CODE FORMAT:
 * The QR code should contain just the asset ID, e.g., "AC-101"
 */
...
export default function ScanQR({ navigation }) {
  ...
  // Loading state while looking up asset
  ...
  if (!permission.granted) {
    return (
      <View style={styles.centerContainer}>
        ...
        {/* Option to enter manually without camera */}
        <TouchableOpacity 
  ...
  /**
   * Handle QR Code Scan
   * Called automatically when camera detects a QR code
   */
  ...
  /**
   * Look up asset in database and navigate
   */
  const lookupAsset = async (assetId) => {
    try {
      // Look up the asset in database
      ...
      // Asset found! Navigate to complaint form
        ...
        <View style={styles.overlay}>
          ...
          <View style={styles.overlayBottom}>
            <Text style={styles.instructions}>
              Point camera at QR code on the machine
            </Text>
            ...
            {/* Scan again button (shown after failed scan) */}
            {scanned && !loading && (
              <TouchableOpacity 
